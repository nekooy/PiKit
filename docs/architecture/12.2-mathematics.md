...
**Two line-box versions are in `docs/verification/formula-renderer.md` and neither is worth
...
build were all free, so **`MathCache` had made a slow renderer non-blocking, not fast.** The two
renderers' per-formula cost, and the cold and warm opens that went with it, are in
[the formula renderer chapter](../verification/formula-renderer.md).
...
per-APK lists are in [the release build's chapter](../verification/release-build.md).
