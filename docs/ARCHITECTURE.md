...
one file per topic — a chapter that outgrew a single topic has numbered parts, `§12.1`
to `§12.3`, and the part is part of the number — because most of the shape of this
project is forced by two external systems that do not bend: the Termux package format
and Pi's RPC protocol.
...
| 3 | [What is baked in](architecture/03-what-is-baked-in.md) | What the runtime image carries, why `rg` and `fd` are not optional, why every version in it — pi's included — is pinned, what the delete list removes from the vendored trees and what it therefore ships |
...
| 9.2 | [The settings pages, and the furniture of a page](architecture/09.2-settings-pages.md) | The header band's fixed height, dividers, the row that acts, forms that ask before leaving, and the three settings pages that needed an argument |
...
a non-obvious decision name the chapter in a comment (`ARCHITECTURE §12.2` and the like),
...
**Every § resolves to one file.** A number without a part is never cited: a chapter
that grew past one topic has parts (`6.1`, `7.3`, `12.2`), the part number is in the
file name, and the table above is what resolves it. An edit that moves a topic between
parts therefore has to re-point the citations in the code and in the documents that
mean it.
