...
- **对话走在 pi 的 RPC 协议上。** 回合结束折叠成一行（`Worked 47s · 5 steps`），点一下重新展开；
  ...
  应用只加了 `!command`、`/new`、`/compact` 和 `/stop` 这四样。
...
`adb install PiKit-<version>-arm64.apk`，并且给首次启动一分钟——它会解包运行时（约 110 MB），
...
运行时镜像是生成物而不是提交物（每个 ABI 约 285 MB），所以那条命令会在途中组装缺失或过期的
镜像。
...
| 文档 | 内容 |
| --- | --- |
| [ARCHITECTURE.md](docs/ARCHITECTURE.md) | 设计决策与背后约束的地图，按章组织 |
| [BUILDING.md](docs/BUILDING.md) | 前置条件、运行时镜像构建、产物、签名、在设备上工作 |
| [VERIFICATION.md](docs/VERIFICATION.md) | 哪些真的跑过、在哪台模拟器和哪台手机上跑过、哪些还没跑 |
| [RELEASING.md](docs/RELEASING.md) | 版本号、tag、工作流，以及发布前必须先完成的签名 |
| [MAINTAINING.md](docs/MAINTAINING.md) | 周期性要做的事：跟进上游 pi、软件包、依赖、设备回归 |
...
| [SECURITY.md](SECURITY.md) | 怎么报告安全问题、哪些算在本项目范围内、哪些该提到上游 |
...
| 组件 | 许可证 | 来源 |
| --- | --- | --- |
| Termux 运行环境与终端 | GPLv3 | [termux/termux-packages](https://github.com/termux/termux-packages)、[termux/termux-app](https://github.com/termux/termux-app) |
| pi，即 Agent（`@earendil-works/pi-coding-agent`） | MIT | [pi.dev](https://pi.dev) |
| `pi-web-access`（Nico Bailon；联网搜索、读取网页、解析 PDF） | MIT | [nicobailon/pi-web-access](https://github.com/nicobailon/pi-web-access) |
