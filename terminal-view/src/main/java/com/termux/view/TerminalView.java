...
public final class TerminalView extends View {
    ...
    public InputConnection onCreateInputConnection(EditorInfo outAttrs) {
        ...
        return new BaseInputConnection(this, true) {
            ...
            void sendTextToTerminal(CharSequence text) {
                // Called from both `commitText` and `finishComposingText`. The
                // former checks for an emulator itself; the latter does not, and
                // an IME that finishes a composition before the view's first
                // layout would otherwise take the app down from inside the input
                // connection — a crash that looks like "typing crashes it".
    ...
    public boolean handleKeyCode(int keyCode, int keyMod) {
        ...
        // `onKeyDown` returns early only when the emulator is missing, and a
        // session can be replaced between the two — a terminal screen reattached
        // to a session that exited in the meantime, for instance. The getter
        // itself returns null until `updateSize` has run, so both are checked
        // rather than assumed.
        TerminalSession session = mTermSession;
        if (session == null || mEmulator == null) return false;
    ...
    private CharSequence getText() {
        // Called from `onScreenUpdated` for the accessibility content description
        // and from the IME's compositing path. Both can run before the first
        // layout has created the emulator.
        if (mEmulator == null) return "";
